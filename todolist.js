class ToDoList {
  constructor() {
    this.inputTyping          = document.querySelector('.header__form input');
    this.addTaskBtn           = document.querySelector('.header__form .btn');
    this.todoList             = document.querySelector('.todo-list');
    this.todoEmpty            = document.querySelector('.todo-empty');
    this.createdTasks         = document.querySelector('.created-tasks span');
    this.completedTasks       = document.querySelector('.created-tasks--completed span');
    this.tasks                = [];
    this.numberTaskCompleted  = 0;

    this.handleEvent();
  }

  handleEvent() {

    document.addEventListener('DOMContentLoaded', () => {
      this.tasks = localStorage.getItem('tasks') ? JSON.parse(localStorage.getItem('tasks')) : [];

      if (this.tasks.length > 0) {
        this.todoEmpty.classList.add('d-none');
      }

      this.createHTML();

    })

    this.addTaskBtn.onclick = (e) => {
      e.preventDefault();
      if (this.addTaskBtn.getAttribute('data-status') == 'create') {
        this.addTask();
      }
      else {
        this.saveTask();
      }
    }

    this.todoList.onclick = (e) => {
      this.deleteTask(e);
      this.editTask(e);
      this.saveStatusTask(e);
    }

  }

  addTask() {

    const task = this.inputTyping.value;

    if (this.inputTyping.value === '') {
      alert("Vui long nhap task...");
      return;
    }

    const taskObj = {
      task: task,
      id: Date.now(),
      status: 'uncompleted'
    }

    this.tasks = [...this.tasks, taskObj];

    console.log(this.tasks);

    if (this.tasks.length > 0) {
      this.todoEmpty.classList.add('d-none');
    }

    this.createHTML();

    this.inputTyping.value = '';

  }

  saveTask() {
    console.log(this.inputTyping.value);

    this.tasks.forEach((task, index) => {
      if (this.editCurrentTask == task.id) {
        this.tasks[index].task = this.inputTyping.value;
        const liEl = document.querySelector(`li[data-id="${task.id}"]`);
        liEl.querySelector('.todo-message').innerHTML = this.inputTyping.value;
      }
    })

    this.addTaskBtn.innerHTML = `<span>Create</span> <img class="btn-icon" src="./assets/image/plus-icon.png"></img>`
    this.addTaskBtn.setAttribute('data-status', 'create');

    this.saveTasksInLocalStorage();

    this.inputTyping.value = '';
  }

  createHTML() {
    this.clearHTML();

    if (this.tasks.length > 0) {
      this.tasks.forEach((task) => {
        const li = document.createElement('li');
        li.innerHTML = `
          <input type="checkbox">
          <p class="todo-message">${task.task}</p>
          <div class="btn-edit">
            <svg class="d-block" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
              <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
              <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/>
            </svg>
          </div>
          <div class="btn-delete">
            <svg class="d-block" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M14.2021 9.98547H12.8716V15.5073H14.2021V9.98547Z" fill="#808080"/>
              <path d="M11.4624 9.98547H10.1318V15.5073H11.4624V9.98547Z" fill="#808080"/>
              <path d="M18.478 7.16712C18.4754 7.03061 18.4295 6.89846 18.3469 6.78975C18.2642 6.68104 18.1492 6.6014 18.0184 6.56232C17.9596 6.53782 17.8974 6.52252 17.8339 6.51696H14.2868C14.1525 6.07791 13.8808 5.69355 13.5117 5.42047C13.1426 5.14739 12.6956 5 12.2365 5C11.7774 5 11.3304 5.14739 10.9613 5.42047C10.5922 5.69355 10.3205 6.07791 10.1862 6.51696H6.63911C6.58068 6.51814 6.52269 6.52729 6.46674 6.54418H6.45162C6.31318 6.58701 6.19334 6.67547 6.11163 6.79515C6.02992 6.91483 5.99117 7.05866 6.00169 7.20319C6.01222 7.34771 6.0714 7.48441 6.16958 7.59099C6.26776 7.69757 6.39916 7.76774 6.54234 7.79006L7.25298 17.5334C7.26382 17.9127 7.41693 18.2741 7.68191 18.5458C7.94688 18.8175 8.30435 18.9797 8.68332 19H15.7867C16.1662 18.9804 16.5244 18.8186 16.79 18.5468C17.0556 18.2751 17.2092 17.9132 17.22 17.5334L17.9277 7.79914C18.0802 7.77797 18.22 7.70232 18.3212 7.58615C18.4223 7.46999 18.478 7.32116 18.478 7.16712ZM12.2365 6.21456C12.3661 6.21458 12.4943 6.24146 12.6129 6.29351C12.7316 6.34556 12.8382 6.42164 12.926 6.51696H11.547C11.6346 6.42135 11.7411 6.34507 11.8599 6.29299C11.9786 6.24092 12.1069 6.21421 12.2365 6.21456ZM15.7867 17.7904H8.68332C8.60168 17.7904 8.47467 17.6573 8.45955 17.4457L7.75798 7.81123H16.715L16.0135 17.4457C15.9984 17.6573 15.8714 17.7904 15.7867 17.7904Z" fill="#808080"/>
            </svg>
          </div>`
          this.todoList.appendChild(li);
          li.classList.add('todo-item');
          li.setAttribute('data-id', task.id);
      })

      const inputCheckbox = document.querySelectorAll('input[type="checkbox"]');
      this.todoItem       = document.querySelectorAll('.todo-item');
      this.todoIcons      = document.querySelectorAll('.btn-delete svg');

      this.tasks.forEach((task) => {
        inputCheckbox.forEach((input) => {
          if (input.closest('.todo-item').getAttribute('data-id') == task.id && task.status == 'completed') {
            input.setAttribute('checked', 'true');
            this.numberTaskCompleted++;
          }
        })
      })

      this.renderNumberTask();
    }

    this.saveTasksInLocalStorage();
  }

  clearHTML() {
    this.todoList.innerHTML = '';
  }

  saveTasksInLocalStorage() {
    localStorage.setItem('tasks', JSON.stringify(this.tasks));
  }

  deleteTask(e) {
    if (e.target.closest('.btn-delete')) {
      this.tasks = this.tasks.filter((task) => {
        return task.id != e.target.closest('.todo-item').getAttribute('data-id');
      })
      this.createHTML();
    }

    if (this.tasks.length <= 0) {
      this.todoEmpty.classList.remove('d-none');
      this.renderNumberTask();
    }
  }

  saveStatusTask(e) {
    if (e.target.tagName == 'INPUT') {
      this.tasks.forEach((task) => {
        if (e.target.closest('.todo-item').getAttribute('data-id') == task.id) {
          task.status = e.target.checked ? 'completed' : 'uncompleted';
        }
      })
    }
    this.createHTML();
  }

  editTask(e) {
    if (e.target.closest('.btn-edit')) {
      this.tasks.forEach((task, index) => {
        if (e.target.closest('.todo-item').getAttribute('data-id') == task.id) {
          this.inputTyping.value = task.task;
          this.tasks[index].task = this.inputTyping.value;
          this.editCurrentTask = task.id;
          this.addTaskBtn.innerHTML = 'Save';
          this.addTaskBtn.setAttribute('data-status', 'edit');
        }
      })
    }
  }

  renderNumberTask() {
    this.createdTasks.innerHTML = this.tasks.length;
    this.completedTasks.innerHTML = `${this.numberTaskCompleted} of ${this.tasks.length}`;
    if (this.tasks.length <= 0) {
      this.createdTasks.innerHTML   = 0;
      this.completedTasks.innerHTML = 0
    }
    this.numberTaskCompleted = 0;
  }

  checkTaskId(id) {
    let isMatch = false;
    this.todoItem.forEach((li) => {
      if(li.getAttribute('data-id') == id) {
        isMatch = true;
      }
    })
    return isMatch;
  }

}

const ToDoListObj = new ToDoList();